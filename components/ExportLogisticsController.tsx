...
interface ExportBooking {
  ...
  shippedDate?: string;
  isoInvoiceNo?: string; // Add ISO ID field
}
...
const DocumentStatusBadge = ({ label, status, type, isoId }: { label: string, status: boolean, type: 'gen' | 'up', isoId?: string }) => (
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-[10px] text-gray-500">
            {status ? (type === 'gen' ? (isoId ? `Generated: ${isoId}` : 'Generated') : 'Uploaded') : 'Pending'}
        </p>
      </div>
    </div>
    {status ? (
...
export const ExportLogisticsController: React.FC<ExportLogisticsControllerProps> = ({ jobs = [], onUpdateJob }) => {
  ...
  const handleMarkShipped = (booking: ExportBooking) => {
    ...
    // 3. Simulate Finance Link (Handled in toggleDocument or implicit)
    console.log("Marked as Shipped");
  };

  const toggleDocument = (bookingId: string, docKey: keyof ExportBooking['documents']) => {
    // If Generating Invoice (CI), create ISO ID
    let isoId = undefined;
    if (docKey === 'ciGenerated') {
        const bk = bookings.find(b => b.id === bookingId);
        // Use first Job ID as master
        const masterJob = bk?.jobs[0]?.id || 'GEN';
        // Use random seq for demo
        isoId = generateISODocID(masterJob, DEPT_CODES.Export, DOC_TYPE_CODES.Invoicing, Math.floor(Math.random()*100));
    }

    setBookings(prev => prev.map(b => {
      if (b.id !== bookingId) return b;
      
      const newVal = !b.documents[docKey];
      
      return {
        ...b,
        isoInvoiceNo: docKey === 'ciGenerated' && newVal ? isoId : b.isoInvoiceNo,
        documents: { ...b.documents, [docKey]: newVal }
      };
    }));
  };
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      {/* ... (Header and Left Column remains same) ... */}
                     ...
                     <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                           ...
                           <div onClick={() => toggleDocument(activeBooking.id, 'ciGenerated')} className="cursor-pointer">
                              <DocumentStatusBadge label="Commercial Invoice (CI)" status={activeBooking.documents.ciGenerated} type="gen" isoId={activeBooking.isoInvoiceNo} />
                           </div>
                        </div>
                     </div>
      ...
      {/* Booking Wizard Modal - (Same as before) */}
      {isBookingModalOpen && (
