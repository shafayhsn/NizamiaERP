...
export const TrimsPlanGenerator: React.FC<TrimsPlanGeneratorProps> = ({ job, onClose, onIssue }) => {
                  ...
                  if (usageRule === 'Generic') {
                      ...
                      style.sizeGroups?.forEach(group => {
                          Object.values(group.breakdown).forEach((row: any) => {
                              Object.entries(row).forEach(([size, q]) => {
                                  // Fix: Cast q to any to avoid unknown type error
                      ...
                      style.sizeGroups?.forEach(group => {
                          group.colors.forEach(col => {
                               ...
                               // Ensure we handle Object.values safely by casting to any[] or properly typing reduce
  ...
  const handleIssuePlan = () => {
      ...
      const requests: PurchasingRequest[] = consolidatedItems.map(item => {
          ...
          // Prepare calculated variant map for PO explosion
        ...
        <div className="px-6 py-4 bg-white border-b border-gray-200 flex justify-between items-center">
           <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                 Consolidated requirements for Job: <span className="font-mono font-bold">{job.id}</span>
              </p>
           </div>
                        ...
                        {consolidatedItems.map(item => {
                            ...
                            return (
                               <tr key={item.key} className="hover:bg-gray-50 transition-colors">
                                  ...
                                  <td className="px-4 py-3 text-blue-600 font-medium text-xs align-top">
                                     {item.supplier}
                                  </td>
                                  <td className="px-4 py-3 text-right font-mono bg-yellow-50/20 text-gray-600 align-top">
                                     {Math.ceil(item.baseRequiredQty).toLocaleString()}
                                  </td>
                                  <td className="px-4 py-3 text-center align-top">
                                     <div className="flex items-center justify-center gap-1 border rounded px-2 py-1 bg-white max-w-[80px] mx-auto focus-within:ring-1 ring-yellow-500">
                                        <input 
                                           ...
                                           value={buffer}
                                           onChange={(e) => handleBufferChange(item.key, e.target.value)}
                                        />
                                        <span className="text-xs text-gray-400">%</span>
                                     </div>
                                  </td>
                                  <td className="px-4 py-3 text-right font-mono font-bold text-green-700 bg-green-50/20 align-top">
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
           ...
           <div className="flex gap-3">
              ...
              <button 
                 ...
                 className="px-5 py-2 bg-yellow-600 text-white text-sm font-medium rounded hover:bg-yellow-700 shadow-sm transition-all flex items-center gap-2 disabled:opacity-50"
              >
