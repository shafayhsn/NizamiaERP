...
export const BulkOrderImportStager: React.FC<BulkOrderImportStagerProps> = ({ onClose, onCommit }) => {
  ...
  const handleCellChange = (rowId: string, field: string, value: string) => {
      setStagedData(prev => prev.map(row => {
          ...
          if (field === 'PO DATE' || field === 'SHIP DATE') {
             // If user types standard date, keep it, otherwise try to parse if they paste DD-MMM-YY
  ...
  const handleImport = () => {
      ...
      selectedRows.forEach(row => {
          ...
          const qty = parseFloat(row.data['ORDER QTY']) || 0;
          const price = parseFloat(row.data['PRICE']) || 0;
          const jobId = row.data['PRODUCT ID'] || `NZ-${Math.floor(Math.random()*10000)}`;
            ...
            <div className="flex-1 overflow-hidden flex flex-col bg-gray-50/30">
                ...
                {step === 1 && (
                    ...
                    >
                        ...
                        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 text-left max-w-4xl w-full">
                            {['Buyer Details', 'Style & PO Info', 'Commercial Dates', 'Price & Qty'].map(label => (
                                <div key={label} className="flex items-center gap-2 text-xs text-gray-500 bg-white p-2 rounded border border-gray-100">
                                    <CheckCircle2 size={14} className="text-green-500" /> {label}
                                </div>
                            ))}
                        </div>
                    </div>
                                    ...
                                    {stagedData.map(row => (
                                        <tr key={row.id} className={`group hover:bg-gray-50 ${row.selected ? 'bg-blue-50/30' : ''}`}>
                                            ...
                                            <td className="p-3">
                                                ...
                                                {row.errors.length > 0 && (
                                                    <div className="text-[9px] text-red-500 mt-1 truncate max-w-[100px]" title={row.errors.join(', ')}>
                                                        {row.errors[0]}
                                                    </div>
                                                )}
                                            </td>
