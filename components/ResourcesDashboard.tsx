...
export const ResourcesDashboard: React.FC<ResourcesDashboardProps> = ({ onOpenTool }) => {
  ...
  const handleToolClick = (toolId: string) => {
    // Direct link to Catalogue Maker & Costing Generator via global modal
    if ((toolId === 'catalogue-maker' || toolId === 'costing-generator') && onOpenTool) {
       onOpenTool(toolId);
       return;
    }

    if (toolId === 'fabric-consumption' || toolId === 'cbm' || toolId === 'sewing-thread' || toolId === 'gsm' || toolId === 'pantone-converter') {
        // Open local modals for calculators if implemented locally, OR use global
        if (onOpenTool) {
            onOpenTool(toolId);
        } else {
